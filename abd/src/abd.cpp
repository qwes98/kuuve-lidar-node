...
{
public:
	LidarSpec(const int deg_of_view, const double resolution)
...
public:
	Abd(const int deg_of_view, const double resolution, const int lambda = 90, const double sigma = 0.005)
		: lidar_spec_(deg_of_view, resolution),
		  lambda_(lambda),
		  sigma_(sigma)
	{}
	//Todo 
	bool getBreakpointArray(const double* const raw_)

private:
	LidarSpec lidar_spec_;
	int lambda_;
	double sigma_;
};
